<template>
  <div class="background">
    <div class="content">
      ...
      <div class="foot">
        <div class="button" @click="register">注册用户</div>
        <div class="button">
          <router-link
            to="/login"
...
const register = async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      ElMessage.error({ message: '注册失败', duration: 1000 })
    }
  } catch (err) {
...
const handleSuccess = (response) => {
  // 获取created中携带的location字段
  imageUrl.value = response.url
  // 读取response.url中末尾的md5zip值作为avatarId
  avatarId.value = response.url.split('/').pop()
